...
var onboardCmd = &cobra.Command{
	Use:   "onboard",
	Short: "Initialize ori configuration and workspace",
	Run:   runOnboard,
}
...
func runOnboard(cmd *cobra.Command, args []string) {
	...
	if configPath == "" {
		...
		configPath = filepath.Join(homeDir, ".ori", "config.json")
	}
...
func printOnboardNextSteps(configPath string) {
	...
	fmt.Printf("  2. Chat: ./ori agent -m \"Hello!\"\n")
	fmt.Println()
...
func (m *wizardModel) View() string {
	...
	s += "\n"
	s += titleStyle.Render("╔══════════════════════════════════════════╗") + "\n"
	s += titleStyle.Render("║     Ori Configuration Wizard        ║") + "\n"
	s += titleStyle.Render("╚══════════════════════════════════════════╝") + "\n"
	s += "\n"
...
func RunWizard() (*config.Config, bool, error) {
	...
	configDir := filepath.Join(usr.HomeDir, ".ori")
