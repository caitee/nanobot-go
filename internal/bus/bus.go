...
// MessageBus is the inbound/outbound message pub/sub surface used by channels,
// cron, the CLI, and the dispatcher. Runtime agent events flow through the
// Dispatcher's own SubscribeRuntimeEvents API and are not part of this bus.
...
// New creates a new MessageBus with the given buffer size for inbound and
// outbound channels.
...
func (b *messageBus) PublishInbound(msg InboundMessage) {
	...
	case <-time.After(time.Second):
		// Drop on timeout. In production this should log.
	}
}
