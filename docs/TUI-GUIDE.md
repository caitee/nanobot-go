...
| `tui_renderer_test.go` | transcript renderer 的纯渲染、normal/detail、tool summary/width 等测试 |
| `tui_render_test.go` | TUI viewport、runtime event、输入路由和整体 View 行为测试 |

### 活动流分层

每次 assistant 响应由下面的 block/segment 类型组成：
...
上面的编号描述类型层级，不代表把同类 segment 集中重排。真实渲染顺序必须以
transcript segment 顺序为准，例如 `reasoning -> tool -> Result preview -> reasoning -> tool -> final text`。
...
tool 详情等内部缩进仍由各自 renderer 负责。

### Viewport Shell 和滚动

`ori agent` 保持普通终端屏幕，而不是 alt screen。启动 banner 会留在宿主终端
历史中；新的可见对话仍统一进入 transcript viewport。这样可以保留启动前后的
终端上下文，并让宿主终端继续负责原生文本选择和复制。

滚动路由遵守下面的规则：

- `PageUp` / `PageDown` 总是滚动 transcript viewport，并把焦点切到 transcript。
- 上下键在没有 slash-command 候选需要移动时作为滚动 fallback；等待模型响应时也可滚动。
- 默认不启用 Bubble Tea 鼠标上报，避免鼠标拖选被 TUI 捕获而无法复制终端文本。
- overlay 打开时，面板按键先交给 overlay，不滚背后的 transcript。

viewport 高度不是固定填满屏幕。`tui_view.go` 记录终端可用上限
`viewportMaxHeight`，每次 transcript 内容变化后用内容行数重新计算当前高度：
内容少时只占实际高度，内容增长时逐步挤占屏幕，超过上限后才在 viewport 内滚动。
这避免一行输入回车后直接清出整屏空白。

### Reasoning 摘要

Reasoning 默认不全量展示。完整 reasoning 保留在 transcript reasoning segment
中；runtime event 和 session replay 只是写入来源之一。`/view` 只改变 renderer
对 transcript reasoning segment 的投影方式，不修改 transcript 数据：
...
- `tool_execution_update` 用于 `detail` 运行中 Preview；没有 partial update 的工具只显示 start/end。
- `detail` Preview/result/error 默认展示最多 4 条非空行，超出时显示 `... N more lines`。
- 工具块只做扫读预览，不替代完整工具结果。
...
- 80 列和窄终端下工具详情行不溢出。
- PageUp/PageDown 和上下键 fallback 都驱动 transcript viewport；overlay 打开时按键先交给面板。
...
├── tui_renderer_test.go # renderer 纯渲染、normal/detail、tool summary/width 测试
└── tui_render_test.go   # viewport、runtime event、输入路由和整体 View 行为测试
```
...
不要默认启用 Bubble Tea 鼠标上报选项。这类模式会让终端把鼠标拖选、滚轮等动作
上报给 Bubble Tea，破坏用户在普通终端屏幕中选中复制 transcript 文本的能力。
滚动 transcript viewport 使用 `PageUp` / `PageDown` 和上下键 fallback。不要把
runtime 输出重新拆回第二套打印平面，也不要在 `View()` 之外直接打印新的可见历史。

onboarding wizard 则使用替代屏幕：
...
1. **reasoning 又全量出现？** 检查 normal 是否只渲染每段标题、detail 是否只渲染尾部摘要。
2. **thinking 标题顺序不对？** 检查 renderer 是否仍按 transcript segment 顺序遍历，不要把多段 reasoning 合并或提前。
3. **detail 工具详情挤成一行？** 检查是否绕过了结构化 tool block renderer；normal 模式只保留一行紧凑 preview，不展开多行详情。
4. **工具动画和底部动画混淆？** 工具运行态不能使用 `spinnerFrames`。
5. **长输出导致布局跳动？** 使用 `lipgloss.Width`、`fitLine` 和 `fitPrefixedLine` 做显式截断。
6. **最终答案重复打印？** 检查 `mergeFinalText`、runtime final 与 outbound fallback 的合并顺序。
7. **长回答时 spinner 卡顿？** 检查是否在 tick 中重复运行完整 transcript render；只有 running tool elapsed 需要 timer-only repaint。
